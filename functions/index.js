...
// Background FCM Trigger: Fires when a document is created in the /notifications collection
  ...
  .onCreate(async (snap, context) => {
    ...
    try {
      ...
      const payload = {
        tokens: tokens,
        notification: {
          title: title || "JobInk Notification",
      ...
      console.log(`Sent FCM notifications to user ${userId}. Success count: ${response.successCount}, Failure count: ${response.failureCount}`);
      ...
      if (response.failureCount > 0) {
        ...
        response.responses.forEach((resp, idx) => {
          if (!resp.success) {
            ...
            if (error && (
              ...
              const tokenRef = db.collection("fcmTokens").doc(badToken);
        ...
        if (pruneCount > 0) {
          await batch.commit();
          console.log(`Pruned ${pruneCount} invalid/expired FCM tokens for user ${userId}.`);
        }
      }
    } catch (err) {
...
// Background FCM Trigger: Fires when a new job is created
exports.onJobCreated = functions.firestore
  .document("jobs/{jobId}")
  .onCreate(async (snap, context) => {
    const job = snap.data();
    if (!job || !job.city) return null;

    try {
      // Query available workers in the same city
      ...
      if (workersSnap.empty) {
        console.log(`No available workers found in city ${job.city} for new job.`);
        return null;
      }

      const workerIds = workersSnap.docs.map(doc => doc.id);
      
      // Fetch FCM tokens for these workers in chunks of 30 due to Firestore query limits
      const tokens = [];
      const chunks = [];
      ...
      for (const chunk of chunks) {
        const tokensSnap = await db.collection("fcmTokens")
          ...
          .get();
        tokensSnap.docs.forEach(doc => tokens.push(doc.id));
      }

      if (tokens.length === 0) {
        console.log(`No FCM tokens registered for available workers in city ${job.city}.`);
        return null;
      }

      // Determine urgency and construct notification payload
      const isUrgent = job.isUrgentExplicit === true;
      const notificationTitle = isUrgent 
        ? "⚡ Immediate Hiring: New Urgent Job!" 
        : "💼 New Job Posted Nearby";
      
      const notificationBody = `${job.title} at ${job.area || job.city}. Pay: ₹${job.payment} (${job.paymentType})`;
      ...
      const payload = {
        ...
        data: {
          ...
          type: isUrgent ? "urgent_job" : "new_job"
        }
      };

      const response = await admin.messaging().sendEachForMulticast(payload);
      console.log(`Dispatched new job notification to ${tokens.length} tokens. Success count: ${response.successCount}`);
      ...
      if (response.failureCount > 0) {
        ...
        if (pruneCount > 0) {
          await batch.commit();
          console.log(`Pruned ${pruneCount} stale tokens during job notification dispatch.`);
        }
      }
    } catch (err) {
      console.error("Error in onJobCreated background notification trigger:", err);
    }
    return null;
  });
